...
AMCAM_EVENT_FPNC               = 0x000f          # fix pattern noise correction status changed
...
AMCAM_OPTION_FAN                    = 0x07       # 0 = turn off the cooling fan, [1, max] = fan speed, set to "-1" means to use default fan speed
...
AMCAM_OPTION_TECTARGET              = 0x0f       # get or set the target temperature of the thermoelectric cooler, in 0.1 degree Celsius. For example, 125 means 12.5 degree Celsius, -35 means -3.5 degree Celsius. Set "-2730" or below means using the default for that model
AMCAM_OPTION_AUTOEXP_POLICY         = 0x10       # auto exposure policy:
                                                   ...
                                                   #     default value: 1
                                                   #
AMCAM_OPTION_FRAMERATE              = 0x11       # limit the frame rate, the default value 0 means no limit
AMCAM_OPTION_DEMOSAIC               = 0x12       # demosaic method for both video and still image: BILINEAR = 0, VNG(Variable Number of Gradients) = 1, PPG(Patterned Pixel Grouping) = 2, AHD(Adaptive Homogeneity Directed) = 3, EA(Edge Aware) = 4, see https://en.wikipedia.org/wiki/Demosaicing
...
AMCAM_OPTION_AFMODE                 = 0x25       # auto focus mode, see AmcamAFMode
AMCAM_OPTION_AFSTATUS               = 0x27       # auto focus status, see AmcamAFStaus
...
AMCAM_OPTION_PRECISE_FRAMERATE      = 0x2d       # precise frame rate current value in 0.1 fps. use AMCAM_OPTION_MAX_PRECISE_FRAMERATE, AMCAM_OPTION_MIN_PRECISE_FRAMERATE to get the range. if the set value is out of range, E_INVALIDARG will be returned
AMCAM_OPTION_BANDWIDTH              = 0x2e       # bandwidth, [1-100]%
AMCAM_OPTION_RELOAD                 = 0x2f       # reload the last frame in trigger mode
AMCAM_OPTION_CALLBACK_THREAD        = 0x30       # dedicated thread for callback: 0 => disable, 1 => enable, default: 0
...
AMCAM_OPTION_AUTOEXPOSURE_PERCENT   = 0x4a       # auto exposure percent to average:
                                                   ...
                                                   #
AMCAM_OPTION_ANTI_SHUTTER_EFFECT    = 0x4b       # anti shutter effect: 1 => enable, 0 => disable; default: 0
...
AMCAM_OPTION_AUTOEXP_EXPOTIME_DAMP  = 0x60       # Auto exposure damp: damping coefficient (thousandths). The larger the damping coefficient, the smoother and slower the exposure time changes
AMCAM_OPTION_AUTOEXP_GAIN_DAMP      = 0x61       # Auto exposure damp: damping coefficient (thousandths). The larger the damping coefficient, the smoother and slower the gain changes
...
AMCAM_OPTION_PSEUDO_COLOR_ENABLE    = 0x65       # Pseudo: -1 => custom: use startcolor & endcolor to generate the colormap
                                                   ...
                                                   #     26 => blue
                                                   #
AMCAM_OPTION_LOW_POWERCONSUMPTION   = 0x66       # Low Power Consumption: 0 => disable, 1 => enable
AMCAM_OPTION_FPNC                   = 0x67       # Fix Pattern Noise Correction
                                                   #     set:
                                                   #         0: disable
                                                   #         1: enable
                                                   #        -1: reset
                                                   ...
                                                   #         ((val & 0xff0000) >> 16): average number
                                                   #
AMCAM_OPTION_OVEREXP_POLICY         = 0x68       # Auto exposure over exposure policy: when overexposed,
                                                   ...
                                                   # Default: 0
AMCAM_OPTION_READOUT_MODE           = 0x69       # Readout mode: 0 = IWR (Integrate While Read), 1 = ITR (Integrate Then Read)
...
AMCAM_OPTION_TAILLIGHT              = 0x6a       # Turn on/off tail Led light: 0 => off, 1 => on; default: on
AMCAM_OPTION_LENSSTATE              = 0x6b       # Load/Save lens state to EEPROM: 0 => load, 1 => save
AMCAM_OPTION_AWB_CONTINUOUS         = 0x6c       # Auto White Balance: continuous mode
                                                   #     0:  disable (default)
                                                   #     n>0: every n millisecond(s)
                                                   #     n<0: every -n frame
AMCAM_OPTION_TECTARGET_RANGE        = 0x6d       # TEC target range: min(low 16 bits) = (short)(val & 0xffff), max(high 16 bits) = (short)((val >> 16) & 0xffff)
AMCAM_OPTION_CDS                    = 0x6e       # Correlated Double Sampling
AMCAM_OPTION_LOW_POWER_EXPOTIME     = 0x6f       # Low Power Consumption: Enable if exposure time is greater than the set value
AMCAM_OPTION_ZERO_OFFSET            = 0x70       # Sensor output offset to zero: 0 => disable, 1 => eanble; default: 0
AMCAM_OPTION_GVCP_TIMEOUT           = 0x71       # GVCP Timeout: millisecond, range = [3, 75], default: 15
                                                   #   Unless in very special circumstances, generally no modification is required, just use the default value
AMCAM_OPTION_GVCP_RETRY             = 0x72       # GVCP Retry: range = [2, 8], default: 4
                                                   #   Unless in very special circumstances, generally no modification is required, just use the default value
AMCAM_OPTION_GVSP_WAIT_PERCENT      = 0x73       # GVSP wait percent: range = [0, 100], default = (trigger mode: 100, realtime: 0, other: 1)
AMCAM_OPTION_RESET_SEQ_TIMESTAMP    = 0x74       # Reset to 0: 1 => seq; 2 => timestamp; 3 => both
AMCAM_OPTION_TRIGGER_CANCEL_MODE    = 0x75       # Trigger cancel mode: 0 => no frame, 1 => output frame; default: 0
AMCAM_OPTION_MECHANICALSHUTTER      = 0x76       # Mechanical shutter: 0 => open, 1 => close; default: 0
AMCAM_OPTION_LINE_TIME              = 0x77       # Line-time of sensor in nanosecond
AMCAM_OPTION_ZERO_PADDING           = 0x78       # Zero padding: 0 => high, 1 => low; default: 0
AMCAM_OPTION_UPTIME                 = 0x79       # device uptime in millisecond
AMCAM_OPTION_BITRANGE               = 0x7a       # Bit range: [0, 8]
AMCAM_OPTION_MODE_SEQ_TIMESTAMP     = 0x7b       # Mode of seq & timestamp: 0 => reset to 0 automatically; 1 => never reset automatically; default: 0
TOUPCAP_OPTION_TIMED_TRIGGER_NUM      = 0x7c       # Timed trigger number
AMCAM_OPTION_TIMED_TRIGGER_LOW      = 0x20000000 # Timed trigger: lower 32 bits of 64-bit integer, nanosecond since epoch (00:00:00 UTC on Thursday, 1 January 1970, see https://en.wikipedia.org/wiki/Unix_time)
AMCAM_OPTION_TIMED_TRIGGER_HIGH     = 0x40000000 # Timed trigger: high 32 bits. The lower 32 bits must be set first, followed by the higher 32 bits
AMCAM_OPTION_AUTOEXP_THLD_TRIGGER   = 0x7d       # trigger threshold of auto exposure
AMCAM_OPTION_LANE                   = 0x7e
...
AMCAM_FRAMEINFO_FLAG_GPS                = 0x00000040   # GPS
AMCAM_FRAMEINFO_FLAG_AUTOFOCUS          = 0x00000080   # auto focus: uLum & uFV
AMCAM_FRAMEINFO_FLAG_COUNT              = 0x00000100   # timecount, framecount, tricount
AMCAM_FRAMEINFO_FLAG_MECHANICALSHUTTER  = 0x00000200   # Mechanical shutter: closed
AMCAM_FRAMEINFO_FLAG_STILL              = 0x00008000   # still image
AMCAM_FRAMEINFO_FLAG_CG                 = 0x00010000   # Conversion Gain: High
...
AMCAM_IOCONTROLTYPE_GET_PWM_FREQ                 = 0x19  # PWM Frequency
AMCAM_IOCONTROLTYPE_SET_PWM_FREQ                 = 0x1a
AMCAM_IOCONTROLTYPE_GET_PWM_DUTYRATIO            = 0x1b  # PWM Duty Ratio
AMCAM_IOCONTROLTYPE_SET_PWM_DUTYRATIO            = 0x1c
AMCAM_IOCONTROLTYPE_GET_PWMSOURCE                = 0x1d  # PWM Source: 0x00 => Opto-isolated input, 0x01 => GPIO0, 0x02 => GPIO1
...
AMCAM_IOCONTROLTYPE_GET_INPUT_STATE              = 0x3b  # Input state: 0 (low level) or 1 (high level)
AMCAM_IOCONTROLTYPE_GET_USER_PULSE_HIGH          = 0x3d  # User pulse high level time: us
AMCAM_IOCONTROLTYPE_SET_USER_PULSE_HIGH          = 0x3e
AMCAM_IOCONTROLTYPE_GET_USER_PULSE_LOW           = 0x3f  # User pulse low level time: us
AMCAM_IOCONTROLTYPE_SET_USER_PULSE_LOW           = 0x40
AMCAM_IOCONTROLTYPE_GET_USER_PULSE_NUMBER        = 0x41  # User pulse number: default 0
AMCAM_IOCONTROLTYPE_SET_USER_PULSE_NUMBER        = 0x42
AMCAM_IOCONTROLTYPE_GET_EXTERNAL_TRIGGER_NUMBER  = 0x43  # External trigger number
AMCAM_IOCONTROLTYPE_GET_DEBOUNCER_TRIGGER_NUMBER = 0x45  # Trigger signal number after debounce
AMCAM_IOCONTROLTYPE_GET_EFFECTIVE_TRIGGER_NUMBER = 0x47  # Effective trigger signal number

AMCAM_IOCONTROL_DELAYTIME_MAX                 = 5 * 1000 * 1000

AMCAM_AFMODE_CALIBRATE      = 0x0   # lens calibration mode
AMCAM_AFMODE_MANUAL         = 0x1   # manual focus mode
AMCAM_AFMODE_ONCE           = 0x2   # onepush focus mode
AMCAM_AFMODE_AUTO           = 0x3   # autofocus mode
AMCAM_AFMODE_NONE           = 0x4   # no active selection of focus mode
AMCAM_AFMODE_IDLE           = 0x5

AMCAM_AFSTATUS_NA           = 0x0   # Not available
AMCAM_AFSTATUS_PEAKPOINT    = 0x1   # Focus completed, find the focus position
AMCAM_AFSTATUS_DEFOCUS      = 0x2   # End of focus, defocus
AMCAM_AFSTATUS_NEAR         = 0x3   # Focusing ended, object too close
AMCAM_AFSTATUS_FAR          = 0x4   # Focusing ended, object too far
AMCAM_AFSTATUS_ROICHANGED   = 0x5   # Focusing ends, roi changes
AMCAM_AFSTATUS_SCENECHANGED = 0x6   # Focusing ends, scene changes
AMCAM_AFSTATUS_MODECHANGED  = 0x7   # The end of focusing and the change in focusing mode is usually determined by the user moderator
AMCAM_AFSTATUS_UNFINISH     = 0x8   # The focus is not complete. At the beginning of focusing, it will be set as incomplete
...
AMCAM_BRIGHTNESS_MIN            = -255     # brightness
AMCAM_BRIGHTNESS_MAX            = 255      # brightness
AMCAM_CONTRAST_DEF              = 0        # contrast
AMCAM_CONTRAST_MIN              = -255     # contrast
AMCAM_CONTRAST_MAX              = 255      # contrast
...
AMCAM_AUTOEXPO_THLD_TRIGGER_DEF = 5        # auto exposure trigger threshold
AMCAM_AUTOEXPO_THLD_TRIGGER_MIN = 2        # auto exposure trigger threshold
AMCAM_AUTOEXPO_THLD_TRIGGER_MAX = 64       # auto exposure trigger threshold
AMCAM_AUTOEXPO_DAMP_DEF         = 0        # auto exposure damping coefficient: thousandths
AMCAM_AUTOEXPO_DAMP_MIN         = 0        # auto exposure damping coefficient: thousandths
AMCAM_AUTOEXPO_DAMP_MAX         = 1000     # auto exposure damping coefficient: thousandths
...
AMCAM_DYNAMIC_DEFECT_T1_MIN     = 0        # dynamic defect pixel correction, dead pixel ratio: the smaller the dead ratio is, the more stringent the conditions for processing dead pixels are, and fewer pixels will be processed
AMCAM_DYNAMIC_DEFECT_T1_MAX     = 100      # means: 1.0
AMCAM_DYNAMIC_DEFECT_T1_DEF     = 90       # means: 0.9
AMCAM_DYNAMIC_DEFECT_T2_MIN     = 0        # dynamic defect pixel correction, hot pixel ratio: the smaller the hot ratio is, the more stringent the conditions for processing hot pixels are, and fewer pixels will be processed
AMCAM_DYNAMIC_DEFECT_T2_MAX     = 100
AMCAM_DYNAMIC_DEFECT_T2_DEF     = 90
...
class AmcamFrameInfoV4:
    def __init__(self):
        self.v3 = AmcamFrameInfoV3()
        self.reserved = 0
        self.uLum = 0
        self.uFV = 0
        self.timecount = 0
...
class Amcam:
    ...
    @classmethod
    def Version(cls):
        """get the version of this dll, which is: 58.28273.20250415"""
        cls.__initlib()
    ...
    def PullImageV4(self, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_PullImageV4(self.__h, pImageData, bStill, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV4()
            self.__lib.Amcam_PullImageV4(self.__h, pImageData, bStill, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV4(pInfo, x)

    def WaitImageV4(self, nWaitMS, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_WaitImageV4(self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV4()
            self.__lib.Amcam_WaitImageV4(self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV4(pInfo, x)
    ...
    def TriggerSyncV4(self, nWaitMS, pImageData, bits, rowPitch, pInfo):
        ...
        """
        if pInfo is None:
            self.__lib.Amcam_TriggerSyncV4(self.__h, nWaitMS, pImageData, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV4()
            self.__lib.Amcam_TriggerSyncV4(self.__h, nWaitMS, pImageData, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV4(pInfo, x)
    ...
    def TriggerSync(self, nWaitMS, pImageData, bits, rowPitch, pInfo):
        ...
        """
        if pInfo is None:
            self.__lib.Amcam_TriggerSync(self.__h, nWaitMS, pImageData, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV3()
            self.__lib.Amcam_TriggerSync(self.__h, nWaitMS, pImageData, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV3(pInfo, x)
    ...
    def get_RealExpoTime(self):
        ...
        x = ctypes.c_uint(0)
        self.__lib.Amcam_get_RealExpoTime(self.__h, ctypes.byref(x))
        return x.value
    ...
    def put_Binning(self, Value, Method):
        self.__lib.Amcam_put_Binning(self.__h, Value, Metho)

    def get_Binning(self):        
        pValue = ctypes.c_char_p(0)
        pMethod = ctypes.c_char_p(0)
        self.__lib.Amcam_get_Binning(self.__h, ctypes.ref(pValue), ctypes.ref(pMethod))
        return (pValue.value, pMethod.value)
        
    def get_BinningNumber(self):
        return self.__lib.Amcam_get_BinningNumber(self.__h)

    def get_BinningValue(self):
        p = ctypes.c_char_p(0)
        self.__lib.Amcam_get_BinningValue(self.__h, ctypes.ref(p))
        return p.value

    def get_BinningMethod(self):
        p = ctypes.c_char_p(0)
        self.__lib.Amcam_get_BinningMethod(self.__h, ctypes.ref(p))
        return p.value
    ...
    def get_TecTargetRange(self):
        ...
        x = ctypes.c_int(0)
        self.__lib.Amcam_get_Option(self.__h, AMCAM_OPTION_TECTARGET_RANGE, ctypes.byref(x))
    ...
    def put_RoiN(self, xOffset, yOffset, xWidth, yHeight):
        ...
        self.__lib.Amcam_put_RoiN(self.__h, pxOffset, pyOffset, pxWidth, pyHeight, Num)
    ...
    def put_SelfTrigger(self, pSt):
        ...
        self.__lib.Amcam_put_SelfTrigger(self.__h, ctypes.byref(x))

    def get_SelfTrigger(self, pSt):
        x = self.__SelfTrigger()
        self.__lib.Amcam_get_SelfTrigger(self.__h, ctypes.byref(x))
        return AmcamSelfTrigger(x.sensingLeft.value, x.sensingTop.value, x.sensingWidth.value, x.sensingHeight.value, x.hThreshold.value, x.lThreshold.value, x.expoTime.value, x.expoGain.value, x.hCount.value, x.lCount.value, x.reserved.value)

    def get_AFState(self):
        x = ctypes.c_uint(0)
        self.__lib.Amcam_get_AFState(self.__h, ctypes.byref(x))
        return x.value

    def put_AFMode(self, mode):
        self.__lib.Amcam_put_AFRoi(self.__h, ctypes.c_uint(mode))

    def put_AFRoi(self, xOffset, yOffset, xWidth, yHeight):
        self.__lib.Amcam_put_AFRoi(self.__h, ctypes.c_uint(xOffset), ctypes.c_uint(yOffset), ctypes.c_uint(xWidth), ctypes.c_uint(yHeight))

    def put_AFAperture(self, iAperture):
        self.__lib.Amcam_put_AFAperture(self.__h, ctypes.c_int(iAperture))

    def put_AFFMPos(self, iFMPos):
        self.__lib.Amcam_put_AFFMPos(self.__h, ctypes.c_int(iFMPos))
    ...
    def FpncOnce(self):
        self.__lib.Amcam_FpncOnce(self.__h)
    ...
    def FpncExport(self, filePath):
        if sys.platform == 'win32':
            self.__lib.Amcam_FpncExport(self.__h, filePath)
        else:
            self.__lib.Amcam_FpncExport(self.__h, filePath.encode())

    def FpncImport(self, filePath):
        if sys.platform == 'win32':
            self.__lib.Amcam_FpncImport(self.__h, filePath)
        else:
            self.__lib.Amcam_FpncImport(self.__h, filePath.encode())

    def IoControl(self, ioLine, eType, outVal):
        x = ctypes.c_int(0)
        self.__lib.Amcam_IoControl(self.__h, ctypes.c_uint(ioLine), ctypes.c_uint(eType), ctypes.c_int(outVal), ctypes.byref(x))
        return x.value
    ...
    def get_FocusMotor(self):
        x = self.__FocusMotor()
        self.__lib.Amcam_get_FocusMotor(self.__h, ctypes.byref(x))
        return AmcamFocusMotor(x.imax.value, x.imin.value, x.idef.value, x.imaxabs.value, x.iminabs.value, x.zoneh.value, x.zonev.value)

    def set_Name(self, name):
        self.__lib.Amcam_set_Name(self.__h, name.encode())

    def query_Name(self):
        str = (ctypes.c_char * 64)()
        self.__lib.Amcam_query_Name(self.__h, str)
    ...
    def put_Name(cls, camId, name):
        cls.__initlib()
        if sys.platform == 'win32':
            return cls.__lib.Amcam_put_Name(camId, name)
        else:
            return cls.__lib.Amcam_put_Name(camId.encode('ascii'), name)
    ...
    def get_Name(cls, camId):
        ...
        str = (ctypes.c_char * 64)()
        if sys.platform == 'win32':
            cls.__lib.Amcam_get_Name(camId, str)
        else:
            cls.__lib.Amcam_get_Name(camId.encode('ascii'), str)
        return str.value.decode()

    @classmethod
    def PixelFormatName(cls, pixelFormat):
        cls.__initlib()
        return cls.__lib.Amcam_get_PixelFormatName(pixelFormat)
    ...
    def __initlib(cls):
        if cls.__lib is None:
            ...
            cls.__lib.Amcam_EnumV2.argtypes = [cls.__DeviceV2 * AMCAM_MAX]
            cls.__lib.Amcam_EnumWithName.restype = ctypes.c_uint
            cls.__lib.Amcam_EnumWithName.argtypes = [cls.__DeviceV2 * AMCAM_MAX]
            cls.__lib.Amcam_put_Name.restype = ctypes.c_int
            cls.__lib.Amcam_get_Name.restype = ctypes.c_int
            ...
            if sys.platform == 'win32':
                ...
                cls.__lib.Amcam_put_Name.argtypes = [ctypes.c_wchar_p, ctypes.c_char_p]
                cls.__lib.Amcam_get_Name.argtypes = [ctypes.c_wchar_p, ctypes.c_char * 64]
                ...
                cls.__lib.Amcam_put_Name.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
                cls.__lib.Amcam_get_Name.argtypes = [ctypes.c_char_p, ctypes.c_char * 64]
            ...
            cls.__lib.Amcam_put_Name.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_Name.errcheck = cls.__errcheck
            cls.__lib.Amcam_Replug.errcheck = cls.__errcheck
            cls.__lib.Amcam_Update.errcheck = cls.__errcheck
            cls.__lib.Amcam_set_Name.restype = ctypes.c_int
            cls.__lib.Amcam_set_Name.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
            cls.__lib.Amcam_set_Name.errcheck = cls.__errcheck
            cls.__lib.Amcam_query_Name.restype = ctypes.c_int
            cls.__lib.Amcam_query_Name.argtypes = [ctypes.c_void_p, ctypes.c_char * 64]
            cls.__lib.Amcam_query_Name.errcheck = cls.__errcheck
            ...
            cls.__lib.Amcam_PullImageV4.restype = ctypes.c_int
            cls.__lib.Amcam_PullImageV4.errcheck = cls.__errcheck
            cls.__lib.Amcam_PullImageV4.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV4)]
            cls.__lib.Amcam_WaitImageV4.restype = ctypes.c_int
            cls.__lib.Amcam_WaitImageV4.errcheck = cls.__errcheck
            cls.__lib.Amcam_WaitImageV4.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV4)]
            ...
            cls.__lib.Amcam_TriggerSyncV4.restype = ctypes.c_int
            cls.__lib.Amcam_TriggerSyncV4.errcheck = cls.__errcheck
            cls.__lib.Amcam_TriggerSyncV4.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV4)]
            ...
            cls.__lib.Amcam_FpncOnce.restype = ctypes.c_int
            cls.__lib.Amcam_FpncOnce.errcheck = cls.__errcheck
            cls.__lib.Amcam_FpncOnce.argtypes = [ctypes.c_void_p]
            ...
            cls.__lib.Amcam_FpncExport.restype = ctypes.c_int
            cls.__lib.Amcam_FpncExport.errcheck = cls.__errcheck
            cls.__lib.Amcam_FpncImport.restype = ctypes.c_int
            cls.__lib.Amcam_FpncImport.errcheck = cls.__errcheck
            if sys.platform == 'win32':
                ...
                cls.__lib.Amcam_FpncExport.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p]
                cls.__lib.Amcam_FpncImport.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p]
            else:
                ...
                cls.__lib.Amcam_FpncExport.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
                cls.__lib.Amcam_FpncImport.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
            ...
            # Value: 1x1, 2x2, etc
            # Method: Average, Add
            cls.__lib.Amcam_put_Binning.restype = ctypes.c_int
            cls.__lib.Amcam_put_Binning.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_char_p]
            cls.__lib.Amcam_put_Binning.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_Binning.restype = ctypes.c_int
            cls.__lib.Amcam_get_Binning.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_char_p), ctypes.POINTER(ctypes.c_char_p)]
            cls.__lib.Amcam_get_Binning.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_BinningNumber.restype = ctypes.c_int
            cls.__lib.Amcam_get_BinningNumber.argtypes = [ctypes.c_void_p]
            cls.__lib.Amcam_get_BinningNumber.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_BinningValue.restype = ctypes.c_int
            cls.__lib.Amcam_get_BinningValue.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.POINTER(ctypes.c_char_p)]
            cls.__lib.Amcam_get_BinningValue.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_BinningMethod.restype = ctypes.c_int
            cls.__lib.Amcam_get_BinningMethod.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.POINTER(ctypes.c_char_p)]
            cls.__lib.Amcam_get_BinningMethod.errcheck = cls.__errcheck
            ...
            cls.__lib.Amcam_put_RoiN.restype = ctypes.c_int
            cls.__lib.Amcam_put_RoiN.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_RoiN.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_uint), ctypes.c_uint]
            ...
            cls.__lib.Amcam_put_SelfTrigger.restype = ctypes.c_int
            cls.__lib.Amcam_put_SelfTrigger.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_SelfTrigger.argtypes = [ctypes.c_void_p, ctypes.POINTER(cls.__SelfTrigger)]
            cls.__lib.Amcam_get_SelfTrigger.restype = ctypes.c_int
            cls.__lib.Amcam_get_SelfTrigger.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_SelfTrigger.argtypes = [ctypes.c_void_p, ctypes.POINTER(cls.__SelfTrigger)]
            cls.__lib.Amcam_get_LensInfo.restype = ctypes.c_int
            cls.__lib.Amcam_get_LensInfo.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_LensInfo.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint)]
            cls.__lib.Amcam_get_AFState.restype = ctypes.c_int
            cls.__lib.Amcam_get_AFState.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_AFState.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint)]
            cls.__lib.Amcam_put_AFMode.restype = ctypes.c_int
            cls.__lib.Amcam_put_AFMode.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_AFMode.argtypes = [ctypes.c_void_p, ctypes.c_uint]
            cls.__lib.Amcam_put_AFRoi.restype = ctypes.c_int
            cls.__lib.Amcam_put_AFRoi.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_AFRoi.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint]
            cls.__lib.Amcam_put_AFAperture.restype = ctypes.c_int
            cls.__lib.Amcam_put_AFAperture.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_AFAperture.argtypes = [ctypes.c_void_p, ctypes.c_int]
            cls.__lib.Amcam_put_AFFMPos.restype = ctypes.c_int
            cls.__lib.Amcam_put_AFFMPos.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_AFFMPos.argtypes = [ctypes.c_void_p, ctypes.c_int]
            cls.__lib.Amcam_get_FocusMotor.restype = ctypes.c_int
            cls.__lib.Amcam_get_FocusMotor.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_FocusMotor.argtypes = [ctypes.c_void_p, ctypes.POINTER(cls.__FocusMotor)]
