...
# --- Detection & Morphology (keep existing knobs) ---
MIN_AREA = 8000
MOG2_HISTORY = 100
...
class EmbryoDetector:
    def __init__(self):
        ...
        # Secondary, more sensitive subtractor
        self.bg_subtractor_sens = cv2.createBackgroundSubtractorMOG2(history=20, varThreshold=50, detectShadows=False)
        ...
        # Edge assist & gating parameters (trimmed but faithful)
        self.AREA_MIN_BANDPASS = 12000
        self.AREA_MAX_BANDPASS = 40000
        self.CONTRAST_MIN_CENTER = 3
        self.CONTRAST_MIN_EDGE = 8
        self.GRAD_MIN_CENTER = 6
        self.GRAD_MIN_EDGE = 9
        self.SOLIDITY_MIN = 0.80
        self.CIRCULARITY_RANGE = (0.35, 0.92)
        self.ASPECT_RATIO_MIN = 0.35
        self.EDGE_SOLIDITY_MIN = 0.82
        self.EDGE_CIRC_MIN = 0.42
        ...
        self.WALL_STRIP_OVERLAP_MAX = 0.30
        self.WALL_STRIP_STRICT_DELTAI = 12
    ...
    def _camera_callback(pData, pInfo, pContext):
        ...
        for poly, area in valid_polys:
            ...
            # Edge-specific: require assist evidence
            if in_edge_band and assist is not None:
                ...
                if int(np.count_nonzero(assist_overlap)) < 120:
                    continue

            # Minimal abs-contrast + gradient gates
            if in_edge_band:
                if deltaI_abs < detector.CONTRAST_MIN_EDGE or grad_mean < detector.GRAD_MIN_EDGE:
                    continue
            else:
        ...
        # print output if embryo is detected
        print("embryo detected")

        detector.detection_count += len(accepts)
        
        for acc in accepts:
            cx, cy = acc["centroid"]
            print(f"[Detection] Frame {fc}: embryo at ({cx}, {cy}), area={acc['area']}")
